    ...
    db.count({ name: data.name }, function (err, count) {
        if (count > 0) {
            db.update({ name: data.name }, {
                $set: { name: data.name, surname: data.surname, city: data.city, phone: data.phone, masterPass: data.masterPass, hash: data.hash, updatedAt: moment().format('MMMM Do YYYY, h:mm:ss a') }
            }, function (err, numReplaced) {
                document.getElementById('alert-p').innerHTML = "Bilgileriniz başarıyla güncellendi.";
                $('#alert').show();
                loadPInfo();
            });
        } else {
            db.insert(data, function (err, newDoc) {
                console.log("bilgiler başarıyla eklendi.");
                loadPInfo();
            });
        }
    });
...
function personalInfoReset() {
    db.remove({
        ...
        document.getElementById('alert-p').innerHTML = "Bilgileriniz başarıyla sıfırlandı.";
        $('#alert').show();
        loadPInfo();
    });
};
